...
function addOpenExtrudedRingSegment(
...
function addClosedExtrudedRingSegment(
  groups: { [name: string]: PrimitiveGroup },
  data: PropertyLoader,
  filterOptions?: FilterOptions
) {
  ...
  globalVertex
    .set(Math.cos(quadOneAngle), Math.sin(quadOneAngle), 0)
    .applyQuaternion(globalAxisRotation)
    .normalize();
  globalVertex1
    .copy(globalVertex)
    .multiplyScalar(data.radiusB)
    .add(globalCenterA);
  globalVertex2
    .copy(globalVertex)
  ...
  globalVertex
    .set(Math.cos(quadTwoAngle), Math.sin(quadTwoAngle), 0)
    .applyQuaternion(globalAxisRotation)
    .normalize();
  globalVertex1
    .copy(globalVertex)
