...
export default abstract class PrimitiveGroup extends GeometryGroup {
  ...
  setVector<T extends TypedArray>(source: THREE.Vector2|THREE.Vector3, target: T, index : number) {
    // @ts-ignore
    if (source.isVector2) {
  ...
  getVector<T extends TypedArray>(array : T, target : THREE.Vector2|THREE.Vector3, index : number) : THREE.Vector2|THREE.Vector3 {
    // @ts-ignore
    if (target.isVector2) {
      // @ts-ignore
      target.set(
