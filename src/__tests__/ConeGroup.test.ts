...
describe('ConeGroup', () => {
  ...
  test('add', () => {
    ...
    const angle = 1.33;
    const arcAngle = 2.33;
    const diagonalSize = Math.sqrt((2 * radiusB) ** 2 + centerA.distanceTo(centerB) ** 2);

    group.add(nodeId, treeIndex, diagonalSize, centerA, centerB, radiusA, radiusB, angle, arcAngle);
