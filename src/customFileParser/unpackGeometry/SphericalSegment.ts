...
function addOpenSphericalSegment(groups: {[name: string]: any}, data: PropertyLoader) {
  groups.SphericalSegment.add(data.nodeId, data.treeIndex, data.center, data.normal, data.radiusA, data.height);
}

function addClosedSphericalSegment(groups: {[name: string]: any}, data: PropertyLoader) {
